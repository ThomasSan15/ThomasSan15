...
# Streak timezone: Bogotá (UTC-5).
# Verified empirically: user.status AC-day streaks match Codeforces display
# under UTC-5. The CF website 63-day streak includes non-AC activity (browsing,
# contest participation) not exposed by user.status, so we show the AC streak
# which is the maximum we can compute from available API data.
...
def build_stats(submissions: list) -> dict:
    ...
    Streak timezone
    ---------------
    Bogotá (UTC-5) is used for day boundaries.
    Verified: this timezone produces the correct AC-day streak matching
    the Codeforces display for this user. The website's streak may differ
    because it counts non-submission activity not available via user.status.
    ...
    ac_days: set = set()        # dates in Bogotá timezone
    ...
    for sub in reversed(submissions):
        ...
        # Collect the submission day in Bogotá time
        ts  = sub.get("creationTimeSeconds", 0)
        day = datetime.fromtimestamp(ts, tz=BOGOTA_TZ).date()
        ac_days.add(day)
    ...
    # ── pass 3: streak (Bogotá timezone, timedelta arithmetic) ────────────────
...
def make_overview_svg(user: dict, stats: dict, contests: list) -> str:
  ...
  <text x="138" y="152" font-family="JetBrains Mono,monospace" font-size="9" fill="#8b949e" letter-spacing="1">AC STREAK</text>
  <text x="280" y="152" font-family="JetBrains Mono,monospace" font-size="9" fill="#8b949e" letter-spacing="1">MAX STREAK</text>
  ...
  <text x="280" y="168" font-family="JetBrains Mono,monospace" font-size="17" fill="#e6edf3" font-weight="700">{max_streak}<tspan font-size="10" fill="#8b949e"> days</tspan></text>
