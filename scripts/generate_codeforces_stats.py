...
def compute_tag_stats(submissions: list) -> dict:
    """
    Count unique solved problems per tag.
    A problem is counted once even if submitted multiple times.
    """
    solved_problems = {}  # (contestId, index) -> set of tags
    for sub in submissions:
        if sub.get("verdict") != "OK":
            continue
...
def make_overview_svg(user: dict, submissions: list, contests: list) -> str:
    ...
    total_solved  = compute_solved_total(submissions)
...
def make_tags_svg(tag_counts: dict, user: dict) -> str:
    handle    = user.get("handle", "")
    rank      = (user.get("rank", "newbie") or "newbie").lower()
    accent    = "#4f8cff"   # blue accent — does NOT change with rank for consistency
    generated = datetime.now(timezone.utc).strftime("%Y-%m-%d")

    # Build display rows: only tracked tags, sorted by solved count desc
    rows = []
    ...
    for i, (tag, count) in enumerate(rows):
        ...
        # Color: full bar gets accent, others get muted blue
...
def main():
    ...
    total_solved = compute_solved_total(submissions)
    print(f"      submissions={len(submissions)}  unique solved={total_solved}")

    tag_counts = compute_tag_stats(submissions)
    ...
    with open(overview_path, "w", encoding="utf-8") as f:
        f.write(make_overview_svg(user, submissions, contests))
    print(f"      ✓ {overview_path}")

    tags_path = os.path.join(args.out, "codeforces_tags.svg")
    with open(tags_path, "w", encoding="utf-8") as f:
        f.write(make_tags_svg(tag_counts, user))
    print(f"      ✓ {tags_path}")
